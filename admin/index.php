...
// ==========================================
// 📊 4. [NEW] ข้อมูลกราฟ 5 สินค้าขายดี (อิงจากจำนวนใน order_details)
// ==========================================
...
foreach($top_products as $tp) {
    // ตัดชื่อยาวๆ ให้สั้นลงเวลากราฟแสดงผล
...
// ==========================================
// 🔔 7. [NEW] กิจกรรมล่าสุด (จำลองดึงลูกค้าใหม่ + ออเดอร์ใหม่ผสมกัน)
// ==========================================
...
<style>
  ...
  .list-group-item-dark { background: transparent; border: none; border-bottom: 1px dashed rgba(255,255,255,0.1); padding: 12px 0; color: #f8fafc; display: flex; align-items: center; gap: 15px; }
  ...
  .timeline-icon { position: absolute; left: -31px; top: 0; width: 20px; height: 20px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 0.7rem; color: #fff; }

  /* 📱 Mobile Adjustments */
  @media (max-width: 767px) {
    .table-responsive { border: 0; }
    .table-dark th, .table-dark td { white-space: nowrap; font-size: 0.85rem; padding: 10px 5px; }
...
<div class="row g-4 mb-4">
  <div class="col-12 col-xl-8 fade-up delay-1">
    <div class="card custom-card shadow-lg h-100">
      ...
      <div class="card-body p-3"><canvas id="salesChart"></canvas></div>
    </div>
  </div>
  <div class="col-12 col-md-6 col-xl-4 fade-up delay-2">
    <div class="card custom-card shadow-lg h-100">
      ...
      <div class="card-body p-3 d-flex justify-content-center"><canvas id="orderStatusChart" style="max-height: 250px;"></canvas></div>
    </div>
  </div>
</div>

<div class="row g-4 mb-4">
  <div class="col-12 col-md-6 col-xl-4 fade-up delay-3">
    <div class="card custom-card shadow-lg h-100">
      ...
      <div class="card-body p-3"><canvas id="topProductsChart" style="max-height: 250px;"></canvas></div>
    </div>
  </div>
      ...
      <div class="card-body p-4 pt-4">
        <ul class="timeline m-0 p-0">
          ...
          <li class="timeline-item">
            ...
            <h6 class="text-white mb-1" style="font-size: 0.9rem;">ออเดอร์ใหม่ #<?= $ro['order_id'] ?></h6>
            <small class="text-muted d-block"><?= $ro['customer_name'] ?> - ฿<?= number_format($ro['total_price']) ?></small>
            <small class="text-secondary" style="font-size: 0.75rem;"><?= date("d/m/y H:i", strtotime($ro['order_date'])) ?></small>
          </li>
          <?php endforeach; ?>
          ...
          <li class="timeline-item">
            ...
            <h6 class="text-white mb-1" style="font-size: 0.9rem;">สมาชิกใหม่สมัครเข้ามา</h6>
            <small class="text-muted d-block"><?= htmlspecialchars($recent_customers[0]['name']) ?></small>
          </li>
          <?php endif; ?>
        </ul>
      </div>
  ...
  <div class="col-12 col-xl-4 fade-up delay-4">
    <div class="card custom-card shadow-lg h-100">
      <div class="card-header border-bottom border-secondary p-3 d-flex justify-content-between align-items-center">
        ...
        <a href="product/products.php" class="btn btn-sm btn-outline-light rounded-pill" style="font-size:0.75rem;">จัดการ</a>
      </div>
      <div class="card-body p-3">
        <ul class="list-group list-group-flush">
          <?php if(empty($low_stock)): ?>
            <div class="text-center py-4 text-muted"><i class="bi bi-check-circle text-success fs-3 mb-2 d-block"></i>สต็อกปกติ</div>
          <?php else: ?>
            <?php foreach($low_stock as $ls): ?>
              <li class="list-group-item list-group-item-dark p-2 border-bottom">
                <img src="product/uploads/<?= htmlspecialchars($ls['p_image'] ?? 'noimg.png') ?>" class="rounded" width="40" height="40" style="object-fit: cover;" alt="img">
                <div class="flex-grow-1 ms-2 overflow-hidden">
                  <h6 class="mb-0 text-truncate text-white" style="font-size: 0.85rem;"><?= htmlspecialchars($ls['p_name']) ?></h6>
                </div>
                <div class="text-end ms-2">
                  <span class="badge <?= $ls['p_stock']==0 ? 'bg-danger' : 'bg-warning text-dark' ?> rounded-pill">เหลือ <?= $ls['p_stock'] ?></span>
                </div>
              </li>
            <?php endforeach; ?>
          <?php endif; ?>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    new Chart(document.getElementById('orderStatusChart').getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: ['#facc15', '#0ea5e9', '#6366f1', '#22c55e', '#ef4444'], borderWidth: 0
        }]
      },
      options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { position: 'bottom', labels: { padding: 15, font: {size: 11} } } } }
    });
    ...
    new Chart(document.getElementById('topProductsChart').getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          label: 'จำนวนชิ้นที่ขายได้', data: <?= json_encode($top_data) ?>,
